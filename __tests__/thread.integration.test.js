...
describe("PUT /api/threads/:id", () => {
  it("Uppdaterar en tråd och kontrollerar Content-Type", async () => {
    ...
    const newThread = await request(app).post("/api/threads").send({
      title: "Gamla titeln",
    ...
    expect(response.body.content).toHaveProperty(
      "content",
      "Uppdaterat innehåll"
    );
  });
...
//Integration test - först skapar vi, sedan raderar vi

describe('DELETE /api/threads/:id', () => {
  it('raderar en tråd och kontrollerar svarsmeddelandet', async () => {
    //Skapar en ny tråd som vi sedan ska radera
    const newThread = await request(app)
    .post('/api/threads')
    .send({
      title: 'Tråd att radera',
      content: 'Innehåll för tråden som ska raderas',
      userId: 1
    });

    //Raderar tråden vi nyss skapat
    const response = await request(app).delete(`/api/threads/${newThread.body.id}`);
    expect(response.status).toBe(200);
